...
const BlogList = ({ blog}) => {
  ...
  return (
    <ul className="border-1 rounded p-4 border-gray-800 shadow-md bg-slate-900 mt-6">
      <li>
        <Link to={`/blog/${blog._id}`}>
          <h2 className="text-2xl font-bold mb-3 cursor-pointer hover:underline">
            {blog.title}
          </h2>
        </Link>
      ...
      <div className="flex justify-between items-center">
        ...
        <div className="flex justify-between gap-1 mt-1 text-sm font-medium items-center">
          ...
          <p className="text-gray-500">{blog?.likes?.length}</p>
        </div>
      </div>
    </ul>
  );
